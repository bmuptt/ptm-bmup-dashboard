...
describe('DialogFormTrainingSchedule', () => {
  ...
  const mountComponent = () => {
    wrapper = mount(DialogFormTrainingSchedule, {
      props: {
        selectData: null,
      },
      global: {
  ...
  test('memilih pelatih dari dialog akan mengisi field pelatih', async () => {
    mountComponent();
    await flushPromises();

    await wrapper.find('[data-testid="open-coach-selection-btn"]').trigger('click');
    await flushPromises();
    ...
    const coachFieldInput = wrapper.find('[data-testid="coach-selection-field"] input');
    expect((coachFieldInput.element as HTMLInputElement).value).toBe('John Doe (johndoe)');
  });
});
